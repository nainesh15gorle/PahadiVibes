...
export default function AipanArtCategory() {
  const [products, setProducts] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const { addToCart } = useCart();

  useEffect(() => {
    fetch("/api/products?category=aipan-art")
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
      ...
      .catch((err) => {
        console.error("Failed to fetch aipan-art products", err);
        setLoading(false);
      });
  }, []);
  ...
  if (loading) {
    return (
      <div className="flex-1 w-full bg-background pt-32 md:pt-40 pb-20 flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <p className="mt-4 text-xs text-primary/70 tracking-widest uppercase">Curating Aipan Art...</p>
      </div>
    );
  }
    ...
    <main className="flex-1 w-full bg-background pt-32 md:pt-40 pb-20">
      <div className="container mx-auto px-4 md:px-8">
        ...
        <div className="text-center mb-16">
          ...
          >
            Discover our exquisite collection of hand-painted Aipan Art. Perfect for adding a touch of traditional Pahadi elegance to your home.
          </motion.p>
        </div>
            ...
            >
              <div className="relative aspect-square overflow-hidden bg-card mb-4 shadow-sm border border-border/50 group-hover:shadow-xl group-hover:border-primary/30 transition-all duration-500">
                ...
                <Link href={`/product/${product.id}`} className="block h-full w-full relative">
                  <Image 
                    src={product.images && product.images.length > 0 ? product.images[0] : "https://images.unsplash.com/photo-1606722590583-6951b5ea92ad"}
                    alt={product.name}
                    fill
                ...
                <div className="absolute bottom-2 right-2 z-20 md:hidden">
                  <button 
                    ...
                    className="w-8.5 h-8.5 rounded-full bg-primary text-primary-foreground flex items-center justify-center shadow-lg active:scale-95 disabled:bg-muted disabled:text-muted-foreground focus:outline-none"
                ...
                <div className="hidden md:block absolute bottom-0 left-0 w-full p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-\\[cubic-bezier(0.22,1,0.36,1)\\] z-20 bg-gradient-to-t from-background/80 to-transparent">
                  <Button 
                    ...
                    disabled={Number(product.stock) === 0}
                    className="w-full shadow-lg rounded-none uppercase tracking-widest text-xs h-12 bg-primary text-primary-foreground hover:bg-primary/90 disabled:bg-muted disabled:text-muted-foreground"
                  >
              ...
              <div className="space-y-1 px-1">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] text-primary uppercase tracking-widest font-semibold">Aipan Art</span>
                ...
                <div className="flex items-center gap-2 pt-0.5">
                  <span className="font-bold text-sm sm:text-base text-foreground">₹{Number(product.price).toLocaleString()}</span>
                  {product.originalPrice && <span className="text-[10px] sm:text-xs text-muted-foreground line-through">₹{Number(product.originalPrice).toLocaleString()}</span>}
                </div>
              </div>
            </motion.div>
          ))}
